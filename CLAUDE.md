...
- **`.github/workflows/assets.yml`** - the single generator for the image assets. Builds the snake into a `dist/` folder and publishes it to a dedicated **`output` branch** with `crazy-max/ghaction-github-pages`, then (if `METRICS_TOKEN` is set) adds the metrics dashboard. The README reads them from `raw.githubusercontent.com/cajogos/cajogos/output/...` (`github-snake-dark.svg`, `metrics.svg`). Runs on push to `main` + daily + dispatch.
  - **Why one workflow:** the publish step *replaces* the entire `output` branch each run, so separate workflows pushing there would clobber each other. One `dist/` build avoids that, keeps `main` free of bot commits, and makes `push` triggers loop-safe (it writes to `output`, never `main`).
  - **Metrics** (`lowlighter/metrics`) is gated on `env.METRICS_TOKEN != ''`, so a missing secret just skips it while the snake still publishes. `METRICS_TOKEN` is a classic PAT with `repo` + `read:org`. It runs **after** the publish step and uses its own committer (`committer_branch: output`) to add `metrics.svg` onto the output branch. Do NOT try to collect metrics into `dist/`: the action renders inside its Docker container (`/metrics_renders/...`), so the file never reaches the workspace and would be lost. Because the publish step replaces the branch each run, metrics re-adds its file every run after that replace. Metrics already includes an isometric contribution calendar (`plugin_isocalendar`), which is why the standalone 3D-contrib graph was removed as redundant.
